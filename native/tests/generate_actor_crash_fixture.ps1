...
$result = "// Generated test oracle: exact crash animation and transitive math; unreached alternate calls abort; do not edit.`n" +
...
foreach ($name in $names) {
    ...
    foreach ($call in [regex]::Matches($found[$name], '(?m)^\s+([A-Za-z0-9_]+)\(rdram, ctx\);')) {
        if ($call.Groups[1].Value -notin $names -and $call.Groups[1].Value -notin $unreached) {
